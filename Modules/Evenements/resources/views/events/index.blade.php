...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y ds">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body collapse {{ request()->hasAny(['type', 'status', 'start_date', 'end_date', 'search']) ? 'show' : '' }}"
                id="filtersCollapse">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="btn-group" role="group">
                        <a href="{{ $events->withQueryString()->url(1) }}"
                            class="btn btn-outline-secondary {{ $events->onFirstPage() ? 'disabled' : '' }}">
                        ...
                        <a href="{{ $events->nextPageUrl() }}"
                            class="btn btn-outline-secondary {{ $events->hasMorePages() ? '' : 'disabled' }}">
                            <i class="fas fa-chevron-right"></i>
                        </a>
                        <a href="{{ $events->withQueryString()->url($events->lastPage()) }}"
                            class="btn btn-outline-secondary {{ $events->hasMorePages() ? '' : 'disabled' }}">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <span
                                            class="text-nowrap">{{ $participantCount }}/{{ $event->max_participants > 0 ? $event->max_participants : '∞' }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="dropdown">
                                        <button type="button"
                                            class="btn btn-icon btn-sm btn-label-secondary dropdown-toggle hide-arrow"
                                            data-bs-toggle="dropdown">
                                            <i class="fas fa-ellipsis-v"></i>
                                        </button>
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item"
                                                href="{{ route('company.evenements.events.show', $event) }}">
                                                <i class="fas fa-eye me-2"></i>Voir
                                            </a>
                                            <a class="dropdown-item"
                                                href="{{ route('company.evenements.events.edit', $event) }}">
                                                <i class="fas fa-edit me-2"></i>Modifier
                                            </a>
                                            @if($event->status == 'published')
                                                <a class="dropdown-item" href="#"
                                                    onclick="event.preventDefault(); document.getElementById('cancel-event-{{ $event->id }}').submit();">
                                                    <i class="fas fa-times-circle me-2"></i>Annuler
                                                </a>
                                                <form id="cancel-event-{{ $event->id }}"
                                                    action="{{ route('company.evenements.events.cancel', $event) }}" method="POST"
                                                    class="d-none">
                                                    @csrf
                                                    @method('PATCH')
                                                </form>
                                            ...
                                            <a class="dropdown-item text-danger" href="#"
                                                onclick="if(confirm('Êtes-vous sûr de vouloir supprimer cet événement ?')) { event.preventDefault(); document.getElementById('delete-event-{{ $event->id }}').submit(); }">
                                                <i class="fas fa-trash-alt me-2"></i>Supprimer
                                            </a>
                                            <form id="delete-event-{{ $event->id }}"
                                                action="{{ route('company.evenements.events.destroy', $event) }}" method="POST"
                                                class="d-none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
            ...
            @if($events->hasPages())
                <div class="card-footer d-flex justify-content-between align-items-center">
                    <div>
                        <p class="mb-0">
                            Affichage de <strong>{{ $events->firstItem() }}</strong> à
                            <strong>{{ $events->lastItem() }}</strong> sur <strong>{{ $events->total() }}</strong> événements
                        </p>
                    </div>
                    <div>
