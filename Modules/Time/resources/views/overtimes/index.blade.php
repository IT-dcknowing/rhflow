        ...
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted mb-0">Gérez les heures supplémentaires des employés pour la période en cours.</p>                        
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0">
                            <li class="breadcrumb-item">
                                <a href="{{ route('company.paiesalaries.dashboard') }}">Paie</a>
                            </li>
                            <li class="breadcrumb-item active">Heures Supplémentaires @if($periode) - Période : {{ $periode->nom }} | Statut : <span class="badge bg-label-{{ $periode->statut === 'en_cours' ? 'success' : ($periode->statut === 'cloture' ? 'secondary' : 'warning') }}">{{ ucfirst($periode->statut) }}</span> @endif</li>
                        </ol>
                    </nav>
                ...
                <div>
                    ...
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#overtimeCreateModal">
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-6">
                            <h4 class="card-title">{{ __('Liste des Heures Supplémentaires') }} ({{$overtimes->count()}})</h4>
                        </div>
                        <div class="col-6 text-end">
                            <a href="{{ route('company.times.overtime.export') }}" class="btn btn-success btn-sm align-items-center">
                            ...
                            <tbody>
                                @foreach($overtimes as $overtime)
                                    <tr>
                                        ...
                                        <td>{{ $overtime->quar_heure + $overtime->heure_audd + $overtime->heure_nuit_ferie + $overtime->heure_dim_ferie + $overtime->heure_nuit_dim_ferie }}</td>
                                        <td>{{ number_format($overtime->montant, 0, ',', ' ') }} {{ __('FCFA') }}</td>
                                        <td>
                                            @if($overtime->paid == 'paid')
                                                <span class="badge bg-success">{{ __('Payé') }}</span>
                                            @else
                                                <span class="badge bg-{{ $overtime->statut == 'approved' ? 'primary' : ($overtime->statut == 'rejected' ? 'danger' : 'warning') }}">
