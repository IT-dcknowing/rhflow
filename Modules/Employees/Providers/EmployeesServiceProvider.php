...
class EmployeesServiceProvider extends ServiceProvider
{
    ...
    {
        $modelsPath = module_path($this->name, 'Models');
        ...
        if (is_dir($modelsPath)) {
            ...
            foreach ($iterator as $file) {
                if ($file->isFile() && $file->getExtension() === 'php') {
                    $modelClass = 'Modules\\' . $this->name . '\\Models\\' . $file->getBasename('.php');

                    if (class_exists($modelClass)) {
                        // Les modèles sont automatiquement enregistrés via l'autoloading PSR-4
