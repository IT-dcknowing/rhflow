...
@section('title', 'Modifier l\'Utilisateur - RH Flow')
...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="mb-1">✏️ Modifier l'Utilisateur</h4>
    ...
    <div class="row">
        <div class="col-xl-10 col-lg-10 mx-auto">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('company.settings.users.update', $user) }}" id="editUserForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('username') is-invalid @enderror"
                                    id="username" name="username" value="{{ old('username', $user->username) }}"readonly required>
                                @error('username')
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="email" class="form-control" name="email" required
                                       placeholder="jean.dupont@exemple.com" value="{{ old('email', $user->email) }}">
                                @error('email')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                                @if($user->id !== auth()->id())
                                    <small class="text-muted">L'utilisateur recevra une notification de changement</small>
                                @else
                                    <small class="text-info">Votre propre email</small>
                                @endif
                            </div>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                @error('password')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">Minimum 8 caractères (laisser vide pour ne pas changer)</small>
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="tel" class="form-control" name="phone"
                                       placeholder="+225 XX XX XX XX" value="{{ old('phone', $user->phone) }}">
                                @error('phone')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" name="type" required onchange="updateTypeDescription()">
                                    @php
                                        $typeLabels = [
                                            'company' => ['label' => '🏢 Entreprise', 'desc' => 'Accès complet à tous les modules'],
                                            'hr' => ['label' => '👥 RH', 'desc' => 'Gestion des employés et congés'],
                                            'payroll' => ['label' => '💰 Paie', 'desc' => 'Gestion des salaires et paies'],
                                            'employee' => ['label' => '👨‍💼 Employé', 'desc' => 'Accès à son profil personnel']
                                        ];
                                    @endphp
                                    <option value="">Choisir un type...</option>
                                    @foreach($typeLabels as $value => $info)
                        ...
                        <div class="row mb-4">
                            <div class="col-12">
                                <h6 class="mb-3">⚙️ Paramètres Supplémentaires</h6>
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="send_notification" value="1" id="sendNotification">
                                    <label class="form-check-label" for="sendNotification">
                                        <strong>Envoyer Notification</strong>
                                    </label>
                                </div>
                                <small class="text-muted">L'utilisateur recevra un email des modifications</small>
                            </div>
                        </div>
                                ...
                                <div class="alert alert-info">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            Créé par {{ $user->creator->name ?? 'Inconnu' }} le {{ $user->created_at->format('d/m/Y à H:i') }}<br>
    ...
    <div class="row mt-4" id="previewSection" style="display: none;">
        <div class="col-xl-10 col-lg-10 mx-auto">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">👁️ Aperçu des Modifications</h5>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="d-flex align-items-center mb-3">
                                ...
                                <div>
                                    <h6 class="mb-0" id="previewName">{{ $user->name }}</h6>
                                    <small class="text-muted" id="previewEmail">{{ $user->email }}</small>
                                </div>
                            </div>

                            <div class="mb-3">
                                <span class="badge bg-label-primary me-2" id="previewType">
                                    @php
                                        $typeLabels = [
                                            'company' => 'Entreprise',
                                            'hr' => 'RH',
                                        ...
                                        echo $typeLabels[$user->type] ?? 'Inconnu';
                                    @endphp
                                </span>
                                <span class="badge {{ $user->is_active ? 'bg-label-success' : 'bg-label-warning' }}" id="previewStatus">
                                    {{ $user->is_active ? 'Actif' : 'Inactif' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                <div class="col-6 mb-2">
                                    ...
                                    <div id="previewPhone">{{ $user->phone ?? '-' }}</div>
                                </div>
                                <div class="col-6 mb-2">
                                    <small class="text-muted">Branche:</small>
                                    <div id="previewBranch">{{ $user->branch->name ?? '-' }}</div>
                                </div>
                                <div class="col-6 mb-2">
                                    <small class="text-muted">Département:</small>
                                    <div id="previewDepartment">{{ $user->department->name ?? '-' }}</div>
                                </div>
                                <div class="col-6 mb-2">
                                    <small class="text-muted">Poste:</small>
                                    <div id="previewDesignation">{{ $user->designation->title ?? '-' }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    @if($user->id !== auth()->id())
                        <div class="alert alert-warning">
                            ...
                            <strong>Attention:</strong> L'utilisateur recevra une notification de ces modifications.
                        </div>
                    @else
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle me-2"></i>
                            <strong>Information:</strong> Vous modifiez votre propre profil.
                        </div>
                    @endif
                </div>
            </div>
...
<script>
    ...
    function updatePreview() {
        const name = document.querySelector('input[name="name"]').value || '{{ $user->name }}';
        const email = document.querySelector('input[name="email"]').value || '{{ $user->email }}';
        const phone = document.querySelector('input[name="phone"]').value || '{{ $user->phone ?? "-" }}';
        const type = document.querySelector('select[name="type"]').value || '{{ $user->type }}';
        ...
        document.getElementById('previewBranch').textContent = branchSelect.options[branchSelect.selectedIndex]?.text || '{{ $user->branch->name ?? "-" }}';
        document.getElementById('previewDepartment').textContent = departmentSelect.options[departmentSelect.selectedIndex]?.text || '{{ $user->department->name ?? "-" }}';
        document.getElementById('previewDesignation').textContent = designationSelect.options[designationSelect.selectedIndex]?.text || '{{ $user->designation->name ?? "-" }}';
    }
