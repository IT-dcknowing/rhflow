...
@section('title', 'Modifier la période : ' . $periode->nom)
            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Modifier la période : {{ $periode->nom }}</h5>
                    ...
                    <div class="card-body">
                        <form action="{{ route('company.paiesalaries.periodes.update', $periode->id) }}" method="POST"
                            id="formPeriode">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row mb-3">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <select class="form-select @error('type_periode') is-invalid @enderror"
                                        ...
                                        <option value="mensuelle" {{ old('type_periode', $periode->type_periode) == 'mensuelle' ? 'selected' : '' }}>Mensuelle</option>
                                        <option value="quinzaine" {{ old('type_periode', $periode->type_periode) == 'quinzaine' ? 'selected' : '' }}>Quinzaine</option>
                                        <option value="hebdomadaire" {{ old('type_periode', $periode->type_periode) == 'hebdomadaire' ? 'selected' : '' }}>Hebdomadaire
                                        </option>
                                    </select>
                                    @error('type_periode')
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="text" class="form-control @error('nom') is-invalid @enderror" id="nom"
                                        name="nom" value="{{ old('nom', $periode->nom) }}" readonly>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="text"
                                        ...
                                        data-min-date="{{ $periode->exercice->date_debut->format('Y-m-d') }}"
                                        data-max-date="{{ $periode->exercice->date_fin->format('Y-m-d') }}" required>
                                    @error('date_debut')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="text"
                                        ...
                                        value="{{ old('date_paiement', $periode->date_paiement->format('Y-m-d')) }}"
                                        data-min-date="{{ $periode->exercice->date_debut->format('Y-m-d') }}"
                                        data-max-date="{{ $periode->exercice->date_fin->format('Y-m-d') }}">
                                    @error('date_paiement')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select @error('statut') is-invalid @enderror" id="statut"
                                        ...
                                        <option value="brouillon" {{ old('statut', $periode->statut) === 'brouillon' ? 'selected' : '' }}>Brouillon</option>
                                        <option value="en_cours" {{ old('statut', $periode->statut) === 'en_cours' ? 'selected' : '' }}>En cours</option>
                                        <option value="validee" {{ old('statut', $periode->statut) === 'validee' ? 'selected' : '' }}>Validée</option>
                                        <option value="payee" {{ old('statut', $periode->statut) === 'payee' ? 'selected' : '' }}>Payée</option>
                                        <option value="annulee" {{ old('statut', $periode->statut) === 'annulee' ? 'selected' : '' }}>Annulée</option>
                                    </select>
                                    @error('statut')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="row mb-3">
                                <div class="col-12 mb-3">
                                    ...
                                    <textarea class="form-control @error('notes') is-invalid @enderror" id="notes"
                                        name="notes" rows="3">{{ old('notes', $periode->notes) }}</textarea>
                                    @error('notes')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
...
@push('scripts')
    ...
    <script>
        $(function () {
            ...
            const exerciceAnnee = "{{ $periode->exercice->nom }}";
            const exerciceDateDebut = new Date("{{ $periode->exercice->date_debut->format('Y-m-d') }}");
            const exerciceDateFin = new Date("{{ $periode->exercice->date_fin->format('Y-m-d') }}");
            ...
            // Initialiser avec le type de période existant
            const typePeriodeInitial = '{{ $periode->type_periode }}';
            if (typePeriodeInitial) {
                mettreAJourSelection(typePeriodeInitial);
            }
