    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="mb-1">⚙️ Configuration des éléments du brut</h4>
                ...
                <div class="d-flex justify-content-end">
                    @if($periode)
                    <a href="{{ route('company.paiesalaries.periodes.show', $periode->id) }}" type="button" class="align-items-center btn btn-info me-2">
                        <i class="fas fa-arrow-left me-2"></i>Retour à la période
                    </a>
                    @endif
                    @if($elements->isEmpty() && $elementsDefault->isNotEmpty())
                    <form id="saveDefaultsForm" method="POST" action="{{ route('company.paiesalaries.allowance.saveDefaults') }}" class="d-flex gap-2">
                        @csrf
                    ...
                    @else
                        <button type="button" class="btn btn-sm btn-primary me-2" data-bs-toggle="modal" data-bs-target="#addElementModal">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item edit-element" href="#" data-allowance-id="{{ $element->id }}" data-allowance-name="{{ $element->name }}"
                                                    data-bs-toggle="modal" data-bs-target="#editElementsModal">
                                                    <i class="fas fa-edit me-2"></i>Modifier
                                                </a>
                                                <div class="dropdown-divider"></div>
                                                <form action="{{ route('company.paiesalaries.allowance.destroyOption', $element->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="dropdown-item text-danger delete-element">
                                    ...
                                    <td class="text-center">
                                        <div class="form-check">
                                            <input class="form-check-input default-element-checkbox" type="checkbox" value="{{ $element->id }}" id="defaultElement{{ $element->id }}" checked disabled>
                                ...
                                @foreach($elementsDefault as $element)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="form-check">
                                                <input class="form-check-input default-element-checkbox" type="checkbox" value="{{ $element->id }}" id="defaultElement{{ $element->id }}">
                                                <label class="form-check-label" for="defaultElement{{ $element->id }}">
                                                    Sélectionner
                                                </label>
                                            </div>
