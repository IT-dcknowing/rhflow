<form id="addRetenueToEmployeesForm" action="{{ route('company.paiesalaries.remboursements.store-employees', $retenue->id) }}" method="POST">
    @csrf
    ...
    <div class="mb-3">
        <h5 class="text-end text-success">+{{ number_format($retenue->amount, 0, ',', ' ') }} FCFA</h5>
        <p class="text-muted text-center">Ces remboursements sont appliqués à <span class="badge bg-label-danger">{{ $retenueCount }}</span> employés</p>
                ...
                <tbody>
                    @forelse($employees as $employee)
                        <tr>
                            ...
                            <td>{{ number_format($employee->get_Autre_retenue_type(), 0, ',', ' ') . ' FCFA' ?? '-' }}</td>
                            <td>
                                @php
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        <a class="dropdown-item" onclick="return confirm('Êtes-vous sûr de vouloir désactiver cette retenue ?')"
                                            href="{{ route('company.paiesalaries.remboursements.deactivate.add', $employeeRetenue->id) }}">
                                        ...
                                        <a class="dropdown-item" onclick="return confirm('Êtes-vous sûr de vouloir activer cette retenue ?')"
                                            href="{{ route('company.paiesalaries.remboursements.activate.add', $employeeRetenue->id) }}">
                                        ...
                                        <a class="dropdown-item" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette retenue ?')"
                                            href="{{ route('company.paiesalaries.remboursements.destroy.add', $employeeRetenue->id) }}">
